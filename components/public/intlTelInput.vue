  ...
  <div class="intl-tel-input allow-dropdown">
    <div class="flag-container">
      <div class="selected-flag" @click="hideSubMenu = !hideSubMenu"
        ...
        <div class="CountryName">{{currentData.name + '  +' + currentData.dialCode}}</div>
        <div class="iti-arrow"></div>
      </div>
      <ul class="country-list" v-show="!hideSubMenu">
        <li @click="currentCode = item.code;hideSubMenu = true;setCountry(item);" v-for="(item, index) in frontCountriesArray" :key="index" :class="'country ' + (item.code == currentCode ? 'highlight' : '')">
          <div class="flag-box">
        ...
        <li class="divider"></li>
        <li @click="currentCode = item.code;hideSubMenu = true;setCountry(item);" v-for="(item, index) in countriesArray" :key="index" :class="'country ' + (item.code == currentCode ? 'highlight' : '')">
          <div class="flag-box">
            <div :class="'iti-flag ' + item.code"></div>
          </div>
          <span class="country-name">{{ item.name }}</span>
...
<script>
  export default (function () {
    const countries = {
      'cn': {'code': 'cn', 'name': 'China (中國)', 'dialCode': 86, 'phoneFormat': '158 8163 9947'},
