<!-- 选择国家地区组件 -->
<template>
  <div class="i18n-tel allow-dropdown">
    <div class="flag-container">
      <div class="selected-flag" @click="hideSubMenu = !hideSubMenu"
        ...
        <div v-if="showCountryName" class="CountryName">{{currentData.name}}</div>
        <div v-if="showDialCode" class="CountryName">+ {{currentData.dialCode}}</div>
        <div class="iti-arrow"></div>
      </div>
      <ul class="country-list" v-show="!hideSubMenu">
        <li @click="currentCode = item.code;hideSubMenu = true;setCountry(item);"
            v-for="(item, index) in frontCountriesArray" :key="index"
        ...
        <li class="divider"></li>
        <li @click="currentCode = item.code;hideSubMenu = true;setCountry(item);"
            v-for="(item, index) in countriesArray" :key="index"
            :class="'country ' + (item.code == currentCode ? 'highlight' : '')">
          <div class="flag-box">
            <div :class="'iti-flag ' + item.code"></div>
          </div>
