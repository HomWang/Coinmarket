var GLOBAL_VAR = {
  ...
  TimeOutId: null,
  button_down: false,
  coinshortName: "BTC",  // coinName 币种名称
  apiSource: null,  // 1: native data, 3: huobi data
};
...
Chart.strMarket = {
  ...
  'kr-ko': {
    '01': '이번주 BTC ', '02': '다음주 BTC ', '03': '월 BTC ', '04': '분기 BTC ',
    '11': '이번주 LTC ', '12': '다음주 LTC ', '13': '월 LTC ', '14': '분기 LTC '
  }
};
Chart.strPeriod = {
  'zh-cn': {
    'line': '(分时)', '0': '(1分钟)', '1': '(5分钟)', '2': '(15分钟)', '9': '(30分钟)', '10': '(1小时)', '3': '(日线)', '4': '(周线)'
  ...
  'zh-tw': {
    'line': '(分時)', '0': '(1分钟)', '1': '(5分钟)', '2': '(15分钟)', '9': '(30分钟)', '10': '(1小時)', '3': '(日线)', '4': '(周线)'
    , '7': '(3分钟)', '11': '(2小時)', '12': '(4小時)', '13': '(6小時)', '14': '(12小時)', '15': '(3天)'
  },
  'kr-ko': {
    'line': '(시간)', '0': '(1분)', '1': '(5분)', '2': '(15분)', '9': '(30분)', '10': '(1시간)', '3': '(일선)', '4': '(주선)'
    , '7': '(3분)', '11': '(2시간)', '12': '(4시간)', '13': '(6시간)', '14': '(12시간)', '15': '(3일)'
  },
};
...
Chart.prototype.updateDepth = function (depth) {
  if (depth == null)
    return;
  if (!depth.asks || !depth.bids || depth.asks == '' || depth.bids == '')
    return;
  var _data = this._depthData;
  _data.array = [];
  for (var i = 0; i < depth.asks.length; i++) {
    var data = {};
    data.rate = depth.asks[i][0];
    data.amount = depth.asks[i][1];
    _data.array.push(data);
  }
  for (var i = 0; i < depth.bids.length; i++) {
    var data = {};
    data.rate = depth.bids[i][0];
    data.amount = depth.bids[i][1];
    _data.array.push(data);
  }
  _data.asks_count = depth.asks.length;
  _data.bids_count = depth.bids.length;
...
DarkTheme.prototype.__construct = function () {
  this._colors = [];
  this._colors[Theme.Color.Positive] = "#4e855a";
  this._colors[Theme.Color.Negative] = "#a5444a";
  this._colors[Theme.Color.PositiveDark] = "#4e855a";
  this._colors[Theme.Color.NegativeDark] = "#a5444a";
  this._colors[Theme.Color.Unchanged] = "#fff";
  this._colors[Theme.Color.Background] = "#25272e"; // 原为 "#0a0a0a"
  ...
  this._colors[Theme.Color.TextPositive] = "#4e855a";
  this._colors[Theme.Color.TextNegative] = "#a5444a";
  ...
  this._colors[Theme.Color.LineColorNormal] = "#a6a6a6";  // 原有的 "#a6a6a6"
...
LightTheme.prototype.__construct = function () {
  ...
  this._colors[Theme.Color.Grid4] = "#aaa";
  this._colors[Theme.Color.TextPositive] = "#db5542";
  this._colors[Theme.Color.TextNegative] = "#53b37b";
...
function KLineMouseEvent() {
  $(document).ready(function () {
    ...
    $('#chart_input_interface').submit(function (e) {
      ...
      // console.log('jsontext', json_text);
    ...
    GLOBAL_VAR.G_HTTP_REQUEST = $.ajax({
      ...
      success: function (json) {
        if (GLOBAL_VAR.G_HTTP_REQUEST) {
          ...
          // huobipro 接口进行数据解析
          if (GLOBAL_VAR.apiSource === 3) {
            const huobiArray = JSON.parse(json);
            const array = [];
            if(huobiArray.length) {
              for (let i = 0; i < huobiArray.length; i++) {
                const eleArray = [
                  huobiArray[i].id * 1000,
                  huobiArray[i].open,
                  huobiArray[i].high,
                  huobiArray[i].low,
                  huobiArray[i].close,
...
var main = function () {
  ...
  window._set_current_language = function (content) {
    console.log('kline set lang: ', content);
    chart_switch_language(content);
  };
  ...
  window._set_api_source = function (apiSource) {
    GLOBAL_VAR.apiSource = apiSource;
  };
  /*
...
function getRealData() {
  ...
  if(!GLOBAL_VAR.KLineData.length) {
    RequestData();
  }
  ...
  if (globalurl.isremote) {
    ...
    var depthCallback = function (data) {
      ...
      // 数据来源: native database
      if (GLOBAL_VAR.apiSource === 1) {
        const result = data.data;
        if(result) {
          result.depth.asks.reverse();
          window._set_current_depth(result.depth);
          window.onPushingResponse(result.period.marketFrom, result.period.type, result.period.coinVol, result.period.data);
        }
      }

      // 数据来源： huobipro，需做数据解析
      if (GLOBAL_VAR.apiSource === 3) {
        // 深度数据
        const depth = data.data.depth;
        depth.asks.reverse();
        window._set_current_depth(depth);
        ...
        window.onPushingResponse(tick.marketFrom, tick.type, tick.coinVol, tickData);
      }
    };
...
$(function () {
  // 请求延后5s, 这个时间要保证RequestData() 请求fullperiod 之后再请求
