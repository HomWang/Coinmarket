  ...
  <section class="container">
    ...
    <section class="NoticeList">
      <section class="NoticeList-Title">
        <span>{{this.NoticeListTitle}}</span>
      ...
      <section class="NoticeList-List">
        ...
        <el-pagination v-show="LoadingState === false" class="NoticeListPagination" layout="prev, pager, next" :page-size="NoticeData.pageSize" :total="NoticeData.totalRows" :current-page="Number(Notice.currentPage)" @current-change="handleCurrentChange"></el-pagination>
      </section>
    </section>
    <About/>
  </section>
...
export default {
  ...
  head () {
    return {
      title: "Coinmarket "+this.$t('common.header.Notice'),
      meta: [
        { hid: 'description', name: 'description', content: 'Coinmarket '+this.$t('common.header.Notice') }
      ]
    }
  },
  mounted(){
    this.Notice.currentPage = window.sessionStorage.getItem('NoticeListIndex')?window.sessionStorage.getItem('NoticeListIndex'):1;
  ...
  methods: {
    handleCurrentChange(val) {
      window.sessionStorage.setItem('NoticeListIndex', val);
    ...
    GetNoticeList(){
      ...
      // 获取公告列表
      Home.noticeList(this.Notice).then((response) => {
        this.NoticeData = response.result;
        this.DateArray = [];
        for(let i = 0; i < response.result.data.length; i++){
          let Time = new Date(response.result.data[i].fcreatedate);
          ...
          let DateObj = {
              fid: response.result.data[i].fid,
              ftitle: response.result.data[i].ftitle,
              fkeyword: response.result.data[i].fkeyword,
